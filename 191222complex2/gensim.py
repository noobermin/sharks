...
def gensim(**kw):
    ...
    if test(kw, "autozipper"):
        files+= ['pzipper','zipper'];
    if not test(kw, "no_loopscript"):
        files.append('loopscript');
    if test(kw, 'sine700points'):
        files.append('sine700points.dat');
    #
    # quick hacks
    #
    if test(kw,'restarts_only'):
        kw.update(
            no_pmovies = True,
            dump_particle = False,
            dump_scalar = False,
    ...
    if test(kw,'singlescale'):
        ...
        unit = getkw('round_unit');
        if test(kw,'roundup_pp'):
            ppf_len = np.ceil(ppf_len/unit)*unit;
            ppb_len = np.ceil(ppb_len/unit)*unit;
            print("rounding up, {},{}".format(ppf_len,ppb_len));
        elif test(kw,'roundup_ten_pp'):
            ppf_len = np.ceil(ppf_len/10/unit) * 10 *unit;
            ppb_len = np.ceil(ppb_len/10/unit) * 10 *unit;
        if type(getkw('fp')) != tuple:
            fpx = get_roundfpx(kw);
            if getkw('fp') != 'nc':
                fpx += getkw('fp');
        ...
        if test(kw,'dens_dat') and kw['dens_dat'] is not None:
            ...
            mytlim = kw['tlim'][:2];
            kw['target_density'] = mktwoscales(n_s, [0.0, slen], mytlim,
                                               Lf, Lb, mytlim, n_fmin, n_bmin);
